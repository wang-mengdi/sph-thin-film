...
{
	{auto p = Add_Object<OpenGLTrackerPoints>("boundary_points");Set_Visibility(p, 'b', true);Set_Color(p, OpenGLColor::White());Set_Point_Size(p, 2.f);}
	{auto p = Add_Object<OpenGLTrackerPoints>("e_tracker_points");Set_Visibility(p, 'T', false);Set_Color(p, OpenGLColor::Orange());Set_Point_Size(p, 2.f);}
	{auto p = Add_Object<OpenGLTrackerCircles>("e_tracker_circles");Set_Visibility(p, 'C', false);
	Set_Shading_Mode(p, ShadingMode::Multicolor);
	if (p)p->Set_Height_File("e_h_bin");
	if (p)p->Set_SA_File("e_sa_bin");
	if (p)p->Set_Radius(0.01);
	if (p)p->Set_Color_Multiplier(1e9);
	}
	{auto p = Add_Object<OpenGLTrackerVectors>("e_point_force");Set_Visibility(p, 'F', false);Set_Color(p, OpenGLColor::Green());Set_Scale(p, default_scale);}
	...
	{auto p = Add_Object<OpenGLTrackerPoints>("l_tracker_points");Set_Visibility(p, 't', false);Set_Color(p, OpenGLColor::Green());Set_Point_Size(p, 1.f);}
	...
	{auto p = Add_Object<OpenGLTrackerCircles>("p_tracker_circles");Set_Visibility(p, 'x', true);
	Set_Shading_Mode(p, ShadingMode::Multicolor);
	if (p)p->Set_Height_File("p_h_bin");
	if (p)p->Set_Radius(0.01);
	if (p)p->Set_Color_Multiplier(1e9);
	}
...
void OpenGLViewerParticleALEFoam::Initialize()
{
	Base::Initialize();
	//Set_Background_Color(OpenGLColor::Black(),OpenGLColor::Black());
	Set_Background_Color(OpenGLColor::Black(), OpenGLColor::Black());
...
void OpenGLViewerParticleALEFoam::Initialize_Data()
{
	{auto p = Add_Object<OpenGLTrackerPoints>("e_tracker_points_0");Set_Visibility(p, 'A', true);Set_Color(p, OpenGLColor::Orange());Set_Point_Size(p, 2.f);}
	{auto p = Add_Object<OpenGLTrackerPoints>("l_tracker_points_0");Set_Visibility(p, 'a', false);Set_Color(p, OpenGLColor::Green());Set_Point_Size(p, 2.f);}
	{auto p = Add_Object<OpenGLTrackerPoints>("e_tracker_points_1");Set_Visibility(p, 'B', true);Set_Color(p, OpenGLColor::Blue());Set_Point_Size(p, 2.f);}
	{auto p = Add_Object<OpenGLTrackerPoints>("l_tracker_points_1");Set_Visibility(p, 'b', false);Set_Color(p, OpenGLColor::Green());Set_Point_Size(p, 2.f);}
	{auto p = Add_Object<OpenGLTrackerPoints>("e_tracker_points_2");Set_Visibility(p, 'C', true);Set_Color(p, OpenGLColor::Red());Set_Point_Size(p, 2.f);}
	{auto p = Add_Object<OpenGLTrackerPoints>("l_tracker_points_2");Set_Visibility(p, 'c', false);Set_Color(p, OpenGLColor::Green());Set_Point_Size(p, 2.f);}
	{auto p = Add_Object<OpenGLTrackerPoints>("e_tracker_points_3");Set_Visibility(p, 'D', true);Set_Color(p, OpenGLColor::Ground_Tan());Set_Point_Size(p, 2.f);}
	{auto p = Add_Object<OpenGLTrackerPoints>("l_tracker_points_3");Set_Visibility(p, 'd', false);Set_Color(p, OpenGLColor::Green());Set_Point_Size(p, 2.f);}
	{auto p = Add_Object<OpenGLTrackerVectors>("e_scalar_ext");Set_Visibility(p, 'P', false);Set_Color(p, OpenGLColor::Yellow());Set_Scale(p, default_scale);}
	{auto p = Add_Object<OpenGLTrackerVectors>("e_point_force_0");Set_Visibility(p, ')', true);Set_Color(p, OpenGLColor::Green());Set_Scale(p, default_scale);}
	{auto p = Add_Object<OpenGLTrackerVectors>("e_point_force_1");Set_Visibility(p, '!', true);Set_Color(p, OpenGLColor::Green());Set_Scale(p, default_scale);}
	{auto p = Add_Object<OpenGLTrackerVectors>("e_point_force_2");Set_Visibility(p, '@', true);Set_Color(p, OpenGLColor::Green());Set_Scale(p, default_scale);}
	{auto p = Add_Object<OpenGLTrackerVectors>("e_point_force_3");Set_Visibility(p, '#', true);Set_Color(p, OpenGLColor::Green());Set_Scale(p, default_scale);}
	{auto p = Add_Object<OpenGLTrackerCircles>("e_tracker_circles_0");Set_Visibility(p, '0', false);
	Set_Shading_Mode(p, ShadingMode::Multicolor);
	if (p)p->Set_Height_File("e_h_bin_0");
	if (p)p->Set_SA_File("e_sa_bin_0");
	if (p)p->Set_Radius(0.03);
	if (p)p->Set_Color_Multiplier(1e9);
	}
	{auto p = Add_Object<OpenGLTrackerCircles>("e_tracker_circles_1");Set_Visibility(p, '1', false);
	Set_Shading_Mode(p, ShadingMode::Multicolor);
	if (p)p->Set_Height_File("e_h_bin_1");
	if (p)p->Set_SA_File("e_sa_bin_1");
	if (p)p->Set_Radius(0.03);
	if (p)p->Set_Color_Multiplier(1e9);
	}
	{auto p = Add_Object<OpenGLTrackerCircles>("e_tracker_circles_2");Set_Visibility(p, '2', false);
	Set_Shading_Mode(p, ShadingMode::Multicolor);
	if (p)p->Set_Height_File("e_h_bin_2");
	if (p)p->Set_SA_File("e_sa_bin_2");
	if (p)p->Set_Radius(0.03);
	if (p)p->Set_Color_Multiplier(1e9);
	}
	{auto p = Add_Object<OpenGLTrackerCircles>("e_tracker_circles_3");Set_Visibility(p, '3', false);
	Set_Shading_Mode(p, ShadingMode::Multicolor);
	if (p)p->Set_Height_File("e_h_bin_3");
	if (p)p->Set_SA_File("e_sa_bin_3");
	if (p)p->Set_Radius(0.03);
	if (p)p->Set_Color_Multiplier(1e9);
	}
	{auto p = Add_Object<OpenGLTrackerCircles>("l_tracker_circles_0");Set_Visibility(p, 'Q', false);
	Set_Shading_Mode(p, ShadingMode::Multicolor);
	if (p)p->Set_Height_File("l_h_bin_0");
	if (p)p->Set_SA_File("l_sa_bin_0");
	if (p)p->Set_Radius(0.03);
	if (p)p->Set_Color_Multiplier(1e9);
	}
	{auto p = Add_Object<OpenGLTrackerCircles>("l_tracker_circles_1");Set_Visibility(p, 'W', false);
	Set_Shading_Mode(p, ShadingMode::Multicolor);
	if (p)p->Set_Height_File("l_h_bin_1");
	if (p)p->Set_SA_File("l_sa_bin_1");
	if (p)p->Set_Radius(0.03);
	if (p)p->Set_Color_Multiplier(1e9);
	}
	{auto p = Add_Object<OpenGLTrackerCircles>("l_tracker_circles_2");Set_Visibility(p, 'E', false);
	Set_Shading_Mode(p, ShadingMode::Multicolor);
	if (p)p->Set_Height_File("l_h_bin_2");
	if (p)p->Set_SA_File("l_sa_bin_2");
	if (p)p->Set_Radius(0.03);
	if (p)p->Set_Color_Multiplier(1e9);
	}
	{auto p = Add_Object<OpenGLTrackerCircles>("l_tracker_circles_3");Set_Visibility(p, 'R', false);
	Set_Shading_Mode(p, ShadingMode::Multicolor);
	if (p)p->Set_Height_File("l_h_bin_3");
	if (p)p->Set_SA_File("l_sa_bin_3");
	if (p)p->Set_Radius(0.03);
	if (p)p->Set_Color_Multiplier(1e9);
	}
	{auto p = Add_Object<OpenGLTrackerPoints>("boundary_points");Set_Visibility(p, '~', true);Set_Color(p, OpenGLColor::White());Set_Point_Size(p, 2.f);}
	{auto p = Add_Object<OpenGLTrackerPoints>("v_tracker_points");Set_Visibility(p, '`', true);Set_Color(p, OpenGLColor::Blue());Set_Point_Size(p, 2.f);}
	//{auto p = Add_Object<OpenGLSegmentMesh>("e_segment_mesh_0");Set_Visibility(p, 'Q', false);Set_Color(p, OpenGLColor::Red());Set_Line_Width(p, 2.f);}
	//{auto p = Add_Object<OpenGLSegmentMesh>("e_segment_mesh_1");Set_Visibility(p, 'W', false);Set_Color(p, OpenGLColor::Red());Set_Line_Width(p, 2.f);}
	//{auto p = Add_Object<OpenGLSegmentMesh>("e_segment_mesh_2");Set_Visibility(p, 'E', false);Set_Color(p, OpenGLColor::Red());Set_Line_Width(p, 2.f);}
}

void OpenGLViewerParticleALEFoamMany::Initialize()
{
	Base::Initialize();
	//Set_Background_Color(OpenGLColor::Black(),OpenGLColor::Black());
	Set_Background_Color(OpenGLColor::Black(), OpenGLColor::Black());
...
void OpenGLViewerParticleALEFoamMany::Initialize_Data()
{
	Array<OpenGLColor> colors = { 
		OpenGLColor::Red(),
		OpenGLColor::Green(),
		OpenGLColor::Ground_Tan(),
		OpenGLColor::Blue(),
		OpenGLColor::Yellow(),
		OpenGLColor::Cyan(),
		OpenGLColor::Magenta(),
		OpenGLColor::Gray(),
		OpenGLColor::Orange(),
		OpenGLColor::Bone_White()
	};
	int num_colors = colors.size();
	for (int i = 0; i < 200; i++) {
		{auto p = Add_Object<OpenGLTrackerPoints>("e_tracker_points_" + std::to_string(i));Set_Visibility(p, 'A', true);Set_Color(p, colors[i%num_colors]);Set_Point_Size(p, 1.f);}
	}
	{auto p = Add_Object<OpenGLTrackerPoints>("boundary_points");Set_Visibility(p, '~', true);Set_Color(p, OpenGLColor::White());Set_Point_Size(p, 1.f);}
}
...
void OpenGLViewerMicroFluidic::Initialize_Data()
{
	...
	{auto p = Add_Object<OpenGLTrackerPoints>("tracker_points"); Set_Visibility(p, 'f', true); Set_Color(p, OpenGLColor::Green());  }
	{auto p = Add_Object<OpenGLTrackerPoints>("trapped_points"); Set_Visibility(p, 'F', true); Set_Color(p, OpenGLColor::Yellow());  }

	////boundary
	{auto p3 = Add_Object<OpenGLParticles<Particles<3> > >("psi_D"); Set_Visibility(p3, 'd', true); Set_Color(p3, OpenGLColor::Red()); }
	{auto p3 = Add_Object<OpenGLParticles<Particles<3> > >("psi_N"); Set_Visibility(p3, 'n', true); Set_Color(p3, OpenGLColor::Blue()); }
}

void OpenGLViewerElastocap::Initialize()
{
	Base::Initialize();
	Set_Background_Color(OpenGLColor::White(), OpenGLColor::White());
}

void OpenGLViewerElastocap::Initialize_Data()
{
	// background grid
	auto grid = Add_Object<OpenGLGrid>("grid"); Set_Visibility(grid, 'g', true);
	// cell type
	auto type = Add_Grid_Scalar_Field("grid", { {"cell_type","",ColorType::Jet,StoreType::Cell} }); Set_Visibility(type, 'T', false);
	// velocity field
	auto vec = Add_Grid_Vector_Field("grid", { {"velocity","1",ColorType::Jet,StoreType::Cell} }); Set_Visibility(vec, 'v', true);

	////solid
	auto c2 = Add_Object<OpenGLSegmentMesh>("segment_mesh_s"); Set_Visibility(c2, 's', true); Set_Color(c2, OpenGLColor::Orange()); Set_Line_Width(c2, 6.f);
	auto c3 = Add_Object<OpenGLTriangleMesh>("triangle_mesh_s"); Set_Polygon_Mode(c3, PolygonMode::SurfOnly); Set_Color(c3, OpenGLColor::Orange()); Set_Visibility(c3, 'S', true);
	auto tri = Add_Object<OpenGLTriangleMesh>("triangle_mesh_v"); Set_Polygon_Mode(tri, PolygonMode::Fill);
	Set_Visibility(tri, 'e', true); Set_Color(tri, OpenGLColor::Orange());
	if (tri) {
	...
	auto tet = Add_Object<OpenGLTetrahedronMesh>("tetrahedron_mesh_v"); Set_Polygon_Mode(tet, PolygonMode::Wireframe);
	Set_Visibility(tet, 'E', true); Set_Color(tet, OpenGLColor::Orange());

	////contact line
	{auto p = Add_Object<OpenGLParticles<Particles<3> > >("contact_points"); Set_Visibility(p, 'c', true); Set_Polygon_Mode(p, PolygonMode::Fill); Set_Color(p, OpenGLColor::Black()); Set_Particle_Mode(p, ParticleMode::Sphere); Set_Particle_Size(p, 1.0f); };
	auto p1 = Add_Object<OpenGLSegmentMesh>("contour"); Set_Visibility(p1, 'C', true); Set_Color(p1, OpenGLColor::Red()); Set_Line_Width(p1, 4.f);

	////fluid
	auto c6 = Add_Object<OpenGLSegmentMesh>("segment_mesh_f"); Set_Visibility(c6, 'f', true); Set_Color(c6, OpenGLColor::Blue()); Set_Line_Width(c6, 4.f);
	auto c7 = Add_Object<OpenGLTriangleMesh>("triangle_mesh_f"); Set_Polygon_Mode(c7, PolygonMode::SurfOnly); Set_Color(c7, OpenGLColor::Blue()); Set_Visibility(c7, 'F', true);
	auto tri_f = Add_Object<OpenGLTriangleMesh>("triangle_mesh_f_seg"); Set_Polygon_Mode(tri_f, PolygonMode::Fill);
	Set_Visibility(tri_f, 'j', true); Set_Color(tri_f, OpenGLColor::Blue());
	if (tri_f) {
		tri_f->recomp_vtx_normal = true;
		tri_f->use_mat = true;
		tri_f->mat.mat_dif = glm::vec4(0.f, 0.f, 1.f, 1.f);
		tri_f->mat.mat_spec = glm::vec4(0.f, 0.f, 1.f, 1.f);
	}
	
	////Set lights
	auto dir_light = OpenGLUbos::Add_Directional_Light(glm::vec3(-1.f, -.1f, -.2f));
	OpenGLUbos::Set_Ambient(glm::vec4(.9f, .9f, .9f, .5f));
	OpenGLUbos::Update_Lights_Ubo();
}
...
{
	Base::Initialize();
	Set_Background_Color(OpenGLColor::White(1.0),OpenGLColor::White(1.0));
}

void OpenGLViewerVoronoi::Initialize_Data()
{	
	{auto c = Add_Object<OpenGLParticles<Particles<3> > >("centroids"); Set_Visibility(c, 'C', true); Set_Point_Size(c, 10.f); Set_Color(c, OpenGLColor::Yellow()); }
	auto grid=Add_Object<OpenGLGrid>("grid");Set_Visibility(grid,'g',true);
	...
	if(use_2d_display){		////ATTENTION: remember to set the option "-d 2" when using the 2D mode!
		{auto c = Add_Object<OpenGLSegmentMesh>("segment_mesh"); Set_Visibility(c, 's', true); Set_Color(c, OpenGLColor(116 / 255., 129 / 255., 176 / 255.)); Set_Line_Width(c, 4.f); }
		auto x = Add_Grid_Scalar_Field("grid", { {"rho","",ColorType::Den,StoreType::Cell} }); Set_Visibility(x, 'x', true); Set_Color(x, OpenGLColor::Black());
		auto x2 = Add_Grid_Scalar_Field("grid", { {"rho2","",ColorType::Den,StoreType::Cell} }); Set_Visibility(x2, 'y', true); Set_Color(x2, OpenGLColor::Yellow());
	}
	else{
		auto x=Add_Grid_Volume("grid",{{"rho","",ColorType::Mat,StoreType::Cell}});Set_Visibility(x,'x',true);
		auto x2=Add_Grid_Volume("grid",{{"rho2","",ColorType::Mat,StoreType::Cell}});Set_Visibility(x2,'y',true);
	...
	auto p2 = Add_Object<OpenGLParticles<Particles<3> > >("particles2"); Set_Visibility(p2, 'Q', true); Set_Point_Size(p2, 10.f); Set_Color(p2, OpenGLColor::Cyan());
	auto p = Add_Object<OpenGLParticles<Particles<3> > >("particles"); Set_Visibility(p, 'P', true); Set_Point_Size(p, 10.f); Set_Color(p, OpenGLColor::Red(1, 0.1));
}
...
void OpenGLViewerTwoPhase::Initialize()
{
	Base::Initialize();
	//Set_Background_Color(OpenGLColor::White(),OpenGLColor::White());
	Set_Background_Color(OpenGLColor::Gray(.4f),OpenGLColor::Gray(.2f));
...
void OpenGLViewerTwoPhase::Initialize_Data()
{
	////grid and fields
	auto grid=Add_Object<OpenGLGrid>("grid");Set_Visibility(grid,'g',true);
	{auto p=Add_Object<OpenGLTrackerPoints>("trial_points");Set_Visibility(p,'1',true);Set_Color(p,OpenGLColor::Green());Set_Point_Size(p,4.f);}
	{auto p=Add_Object<OpenGLTrackerPoints>("spray_points");Set_Visibility(p,'2',true);Set_Color(p,OpenGLColor::Blue());Set_Point_Size(p,4.f);}
	{auto p=Add_Object<OpenGLTrackerPoints>("foam_points");Set_Visibility(p,'3',true);Set_Color(p,OpenGLColor::Yellow());Set_Point_Size(p,4.f);}		////fields
	{auto vec=Add_Grid_Vector_Field("grid",{{"velocity","",ColorType::Jet,StoreType::Cell}});Set_Visibility(vec,'v',true);Set_Scale(vec,default_scale);}
	{auto psi_L=Add_Grid_Vector_Field("grid",{{"psi_L","",ColorType::Jet,StoreType::Cell}});Set_Visibility(psi_L,'L',false);Set_Scale(psi_L,default_scale);}
	{auto psi_A=Add_Grid_Vector_Field("grid",{{"psi_A","",ColorType::Jet,StoreType::Cell}});Set_Visibility(psi_A,'A',false);Set_Scale(psi_A,default_scale);}
	{auto spin=Add_Grid_Vector_Field("grid",{{"spin","",ColorType::Jet,StoreType::Cell}});Set_Visibility(spin,'A',true);Set_Scale(spin,default_scale);}
	{auto type=Add_Grid_Scalar_Field("grid",{{"fluid_type","",ColorType::Jet,StoreType::Cell}});Set_Visibility(type,'T',false);}
	...
	{auto pressure=Add_Grid_Scalar_Field("grid",{{"pressure","",ColorType::Jet,StoreType::Cell}});Set_Visibility(pressure,'P',false);}

	////mesh for solid
	auto c2=Add_Object<OpenGLSegmentMesh>("solid_segment_mesh");Set_Visibility(c2,'z',true);Set_Color(c2,OpenGLColor::Green(.5f));Set_Line_Width(c2,4.f);
	auto c3=Add_Object<OpenGLTriangleMesh>("solid_triangle_mesh");Set_Polygon_Mode(c3,PolygonMode::Wireframe);Set_Shading_Mode(c3,ShadingMode::None);
	Set_Color(c3,OpenGLColor::Red());Set_Visibility(c3,'x',true);Set_Line_Width(c3,4.f);

	////mesh for levelset
	c2=Add_Object<OpenGLSegmentMesh>("segment_mesh");Set_Visibility(c2,'s',true);Set_Color(c2,OpenGLColor::Red());Set_Line_Width(c2,4.f);
	c3=Add_Object<OpenGLTriangleMesh>("triangle_mesh");Set_Polygon_Mode(c3,PolygonMode::Wireframe);Set_Shading_Mode(c3,ShadingMode::None);
	Set_Color(c3,OpenGLColor::Green(.5f));Set_Visibility(c3,'t',true);Set_Line_Width(c3,4.f);
	...
	////tracker points
	{auto p=Add_Object<OpenGLTrackerPoints>("tracker_points");Set_Visibility(p,'R',true);Set_Color(p,OpenGLColor::Green());Set_Point_Size(p,2.f);}
	{auto p=Add_Object<OpenGLTrackerVectors>("tracker_psi");Set_Visibility(p,'V',true);Set_Color(p,OpenGLColor::Blue());Set_Scale(p,default_scale);}
	//{auto p=Add_Object<OpenGLTrackerVectors>("point_velocity");Set_Visibility(p,'V',true);Set_Color(p,OpenGLColor::Blue());Set_Scale(p,default_scale);}


	////velocity dbg
	{auto vec=Add_Grid_Vector_Field("grid",{{"psi_velocity","",ColorType::Jet,StoreType::Cell}});Set_Visibility(vec,'K',true);Set_Scale(vec,default_scale);}
	{auto p=Add_Object<OpenGLTrackerVectors>("face_velocity");Set_Visibility(p,'A',true);Set_Color(p,OpenGLColor::Blue());Set_Scale(p,default_scale);}
	{auto p=Add_Object<OpenGLTrackerVectors>("face_psi_velocity");Set_Visibility(p,'B',true);Set_Color(p,OpenGLColor::Green());Set_Scale(p,default_scale);}
	{auto p=Add_Object<OpenGLTrackerPoints>("face_flag");Set_Visibility(p,'U',true);Set_Color(p,OpenGLColor::White());Set_Point_Size(p,8.f);}
}
...
void OpenGLViewerRigid::Initialize()
{
	Base::Initialize();
	Set_Background_Color(OpenGLColor::Gray(.4f), OpenGLColor::Gray(.2f));
}

void OpenGLViewerRigid::Initialize_Data()
{
	auto grid = Add_Object<OpenGLGrid>("grid"); Set_Visibility(grid, 'g', true);
	auto type = Add_Grid_Scalar_Field("grid", { {"cell_type","",ColorType::Jet,StoreType::Cell} }); Set_Visibility(type, 'T', false);
	auto vec = Add_Grid_Vector_Field("grid", { {"velocity","1",ColorType::Jet,StoreType::Cell} }); Set_Visibility(vec, 'v', true);
...
void OpenGLViewerThinShell::Initialize()
{
	Base::Initialize();
	Set_Background_Color(OpenGLColor::White(), OpenGLColor::White());
}

void OpenGLViewerThinShell::Initialize_Data()
{
	if (use_2d_display)
	{
		auto s = Add_Object<OpenGLSegmentMesh>("segment_mesh"); Set_Polygon_Mode(s, PolygonMode::Wireframe); Set_Visibility(s, 's', true); 
		Set_Color(s, OpenGLColor::Black());
		Set_Shading_Mode(s, ShadingMode::None);
	}else{ 
		auto t = Add_Object<OpenGLTriangleMesh>("triangle_mesh"); Set_Polygon_Mode(t, PolygonMode::Wireframe); Set_Shading_Mode(t, ShadingMode::None);
		Set_Color(t, OpenGLColor::Black()); Set_Visibility(t, 't', true);
		auto rhos = Add_Mesh_Scalar_Field(t, { {"rhos","",ColorType::Jet,StoreType::Node} }); Set_Visibility(rhos, 'd', true);
	}
